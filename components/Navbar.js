          ...
          {session ? (
            <div className="relative group">
              {session.user.avatar_url ? (
                <img src={session.user.avatar_url} alt="Avatar" className="w-8 h-8 rounded-full" />
              ) : (
                <span className="text-sm cursor-pointer hover:underline">{session.user.email}</span>
              )}
            ...
            <Link href="/login">
              <a className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-1 rounded text-sm">Log In</a>
            </Link>
          )}
        </div>
      </div>
